...
/*
 *  using BFS here because it naturally breaks the problem into levels, by
 * keeping track and count per level, we compute averages efficiently in O(N)
 * time T: O(n), S: O(n)
 */
...
/*
 *  using DFS here because we can break the problem into each subtree
 * contributes its; local left-leaf sum T: O(n), S: O(h)
 */
...
/*
 *  using dynamic programming here becuase we can break the problem into smaller
 * subproblems, relying only on already computed smaller states. T: O(n), S:
 * O(n)
 */
...
/*
 *  using Bezout's identity here because we can break the problem into multiple
 * of gcd T: O(n), S: O(1)
 */
...
void findLaddersbfs(const string &beginWord, const string &endWord,
 ...
 *  T: O(N * L^2), S: O(N * L)
 */
vector<vector<string>> Purgatory::findLadders(string beginWord, string endWord,
...
/*
 *  using preorder DFS here because it's symmetrical and ensures that
 * serialization and deserialization mirror each other
 */
