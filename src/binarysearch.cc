...
/*
 *  using binary search here because the problem reduces to finding the first
 * index where nums[i] >= target. Binary search guarantees O(log n) complexity.
...
/*
 *  using binary search across the flattened matrix here because the matrix
 * properties let us treat it as a sotred 1D array. The state is defined by the
 * search range [left, right]. Mapping mid back into (row, col) allows us to
 * compare correctly. This give O(log m * n) complexity. T: O(log (m * n)), S:
 * O(1)
 */
...
/*
 *  using binary search on partitions here because we can break the problem into
 * finding the correct split point T: O(log min(m, n)), S: O(1)
 */
double Purgatory::findMedianSortedArrays(vector<int> &nums1,
                                         vector<int> &nums2) {
...
/*
 *  using Floy'd algorithm here because we can break the problem into the array
 * into a function f(i) = nums[i] T: O(n), S: O(1)
 */
...
/*
 *  using binary search with duplicate-skip shrink here because we can break the
 * problem into the array as two sorted parts separated by rotation T: O(log n),
 * S: O(1)
 */
...
/*
 *  using binary search with duplicate handling here because we can break the
 * problem into in a rotated sorted array, the direction of mod relative to
 * right tell us which half is unsorted. T: O(log n), S: O(n)
 */
