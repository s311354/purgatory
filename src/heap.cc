...
/*
 *  using a max-heap here because the main operation is repeatedly taking the
 * two largest elements. We break the problem into
...
/*
 *  using DP with three pointers here because we can break the problem into
 * multiplying previous ugly numbers by 2, 3, 5, and then always taking the
 * minimum
 */
...
/*
 *  using a monatomix deque here because it allows us to maintain the max
 * efficiently as the window slides. Each element is processed at most twice.
 */
...
vector<int> Purgatory::getOrder(vector<vector<int>> &tasks) {
  ...
  sort(sortedTasks.begin(), sortedTasks.end(),
       [](const TaskGetOrder &a, const TaskGetOrder &b) {
