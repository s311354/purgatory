...
vector<string> Purgatory::letterCombinations(string digits) {
  ...
  // cache behavior
  static const string mapping[10] = {"",    "",    "abc",  "def", "ghi",
...
/*
 *  using backtracking here because the problem is about combinatorial
 * generation. We define the recursion state (start, path), and stop once path
 * size == k
 */
void dfsCombine(int start, int depth, int n, int k, vector<int> &path,
...
/*
 *  using backtracking with pruning here because the state is (row, occupied
 * cols, diagonals, anti-diagonals). At each row, we explore possible
 * placements.
 */
int Purgatory::totalNQueens(int n) {
  ...
  // cpu pipeline
  function<void(int, int, int, int)> dfs = [&](int row, int cols, int diag,
                                               int antiDiag) {
    if (row == n) {
      count++;
      return;
...
/*
 *  using a bit counting abstraction here because we can break the problem into
 * bit combinatorics T: O(1), S: O(K)
 */
...
/*
 *  using recursion here because we can break the problem into the state space
 * of subsets. T: O(n*2^n), S:O(n*2^n)
 */
void dfsSubsets(int start, int n, vector<int> &nums, vector<int> &current,
                vector<vector<int>> &result) {
  // cache behavior
...
/*
 *  using built by reflection here because we can break the problem into
 * reflection + bit prefixing T: O(2^n), S:O(2^n)
 */
...
void dfsFindSubsequences(vector<int> &nums, int start, vector<int> &curr,
                         vector<vector<int>> &result) {
