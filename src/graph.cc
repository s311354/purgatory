...
/*
 *  using a degree-based counting method here because we can break the problem
 * into
    ...
    for (int j = 0; j < n; ++j) {
      ...
      while (!st.empty()) {
        ...
        for (int k = 0; k < 4; ++k) {
          ...
          // branch prediction
          if ((unsigned)nr < (unsigned)m && (unsigned)nc < (unsigned)n &&
              grid[nr][nc] == '1') {
...
/*
 *  using DFS + memorization here because we can break the problem into
 * overlapping subproblems: each cell's longest path depends only on its
 * neighbors T: O(m*n), S: O(m*n)
 */
  ...
  while (!q.empty()) {
    ...
    while (sz--) {
      ...
      for (int k = 0; k < 4; ++k) {
        ...
        // branch prediction
        if ((unsigned)nr < (unsigned)m && (unsigned)nc < n &&
            matrix[nr][nc] < matrix[r][c]) {
          if (--memo[nr][nc] == 0)
            q.emplace(nr, nc);
        }
