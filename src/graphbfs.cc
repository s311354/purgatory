...
/*
 *  using BFS level order traversal here because we can break the problem into
 * layers. The trick is: instead of reversing at the end, we insert each new
 * level at the begining of the result vector. T: O(n), S: O(n)
 */
...
/*
 *  using a constant space solution by exploiting the perfect binary tree
 * property here because next pointers already built to traverse levels T: O(n),
 * S: O(1)
 */
...
/*
 *  using BFS here because it naturally guarantees minimum removals, once we
