...
/*
 *  using counting external edges here because we can break the problem into
 * each cell starts with 4 potential edges and every shared edge between two
 * adjacent lands removes 2 total edges (one from each cell) T: O(rows * cols),
 * S: O(1)
 */
...
/*
 *  using heap-based merge of sorted lists here because we can break the problem
 * into n sorted lists. T: O(k log n), S: O(n)
 */
// cache behavior
...
/*
 *  using pattern recognition from invariants here because we can break the
 * problem into that each ship's head is the only unique indicator. T: O(mn), S:
 * O(1)
 */
...
/*
 *  using histogram-based dynamic accumulation approach here because we can
 * break the proble into an extension of the largest rectangle in histogram
 * problem. T: O(m*n), S: O(n)
 */
